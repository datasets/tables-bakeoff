 ...
 * the site's own `large` (500,000-row) Parquet fixture — chosen so
 * aggregation queries actually demonstrate DuckDB's speed, which a
 * 1,000-row dataset wouldn't. Deliberately outside the mountDemo contract:
 * this page has interactive state (query text, results) that persists
 * across re-renders, not four fixed per-dataset render functions. */
...
root.innerHTML = `
  ...
  <header class="wrap" style="padding-top:22px;padding-bottom:10px">
    ...
    <p style="margin:0;max-width:70ch;color:var(--text-secondary);font-size:14px">
      ...
      site's own 500,000-row dataset with DuckDB-WASM, rendered into the
